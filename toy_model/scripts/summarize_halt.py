...
runs = sorted(r for r in glob.glob(os.path.join(root, "halt_*_s*")) if re.search(r"halt_(D|H|P|Hc)_s\d+$", r))
lo = lambda blk, d, k="correct": min(blk[f"one_new|d{d}"][k], blk[f"random|d{d}"][k])
print("== FINAL (full 5,000 per cell): min over one_new / random; fit = all_seen d2")
for r in runs:
    p = os.path.join(r, "final_eval.json"); name = os.path.basename(r)[5:]
    if not os.path.exists(p):
        continue
    j = json.load(open(p)); m = j["main"]; sh = j["shallow"]
    print(f"{name:9s} atomic {sh['atomic']['correct']:.3f} val_d2 {sh['val_d2']['correct']:.3f} fit {m['all_seen|d2']['correct']:.3f} | main " + " ".join(f"d{d}:{lo(m, d):.3f}" for d in DEPTHS))
    print(f"{'':9s} forced R=d                          | " + " ".join(f"d{d}:{lo(j['forced_R_eq_d'], d):.3f}" for d in DEPTHS))
    if "timeout" in m["one_new|d2"]:
        print(f"{'':9s} timeout / wrong stop / mean T      | " + " ".join(f"d{d}:{m[f'one_new|d{d}']['timeout']:.2f}/{m[f'one_new|d{d}']['wrong_stop']:.2f}/{m[f'one_new|d{d}']['T']:.1f}" for d in DEPTHS))
    g = j["fixed_grid_first500"]; print(f"{'':9s} best fixed R on one_new (R:acc)     | " + " ".join(f"d{d}:" + max(((int(k[1:]), v[f'one_new|d{d}']['correct']) for k, v in g.items()), key=lambda t: t[1]).__repr__() for d in DEPTHS))
...
for r in runs:
    ...
    line = f"{name:9s} u{l['update']:7d} ({l['elapsed'] / 3600:.1f} h) atomic {sh['atomic']['correct']:.3f} val_d2 {sh['val_d2']['correct']:.3f} fit {sh['train_d2_fit']['correct']:.3f} | " + " ".join(f"d{d}:{lo(m, d):.2f}" for d in DEPTHS)
    if "T" in m["one_new|d2"]:
        line += " | T " + "/".join(f"{m[f'one_new|d{d}']['T']:.0f}" for d in DEPTHS) + f" | timeout d2 {m['one_new|d2']['timeout']:.2f} d8 {m['one_new|d8']['timeout']:.2f}"
