...
| 作业 | 内容 | walltime | 状态 |
|---|---|---|---|
| **3391720–3391739 lat2-01 … lat2-20** | **每个 run 独占一个节点**（run id = 作业名后两位，顺序同 `configs/cells_latent2.txt`），无 MPS，记在 **gj26** 项目下（`-W group_list=gj26`；用户 09-18 21:45 批准使用 gj26 并要求一节点一任务）。单进程 48.5 ms / update → 250k ≈ 3.4 h，加监测与 final 评估约 3.6 h | 5 h | 全部完成 09-19（A–D 3.3–3.6 h、F 3.6–3.7 h、E 1.3–1.4 h；fail=0） |
...
20 个 run 全部完成（作业 3391720–3391739，09-19 00:21 – 约 09:00，全部 fail=0，无 traceback）。设置与数字的完整汇总见 **[results_latent_batch2_2026-09-19.md](results_latent_batch2_2026-09-19.md)**。要点：训练关系对拟合 0.99–1.00；**未见关系对的 d2 在全部 20 个 run 上是 chance（0.001–0.003）**，d3–d128 同样；预注册判据全部 0 通过；实体 / 读取监督主效应、交互、A − E、A − F 全部在 ±0.001 内。读取监督能把读取顺序迁移到未见关系对（命中 0.6–1.0），但答案仍是 chance；单任务原子题 0.3%–12%；A–D 的 seed 1 / 7 在背 w2 行；T = 256 只会让已拟合的训练对变差（D 降到 0.13–0.66）。配方未改。
