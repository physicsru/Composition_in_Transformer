...
| 作业 | 内容 | walltime | 状态 |
|---|---|---|---|
| 3391711 lat2-a | run 01 / 05 / 08 / 11 / 15（A–E 的 seed 1）+ 02 / 06 / 09 / 12 / 16（A–E 的 seed 7），10 个进程同卡，`MPS=1` | 6.5 h | 排队 09-18 21:35 |
| 3391712 lat2-b | run 03 / 07 / 10 / 13 / 17（A–E 的 seed 123）+ 04 / 14（A、D 的 seed 2026）+ 18 / 19 / 20（F × 3），10 个进程同卡，`MPS=1` | 6.5 h | 排队 09-18 21:35 |
...
- **打包**：按用户要求 20 个 run 在吞吐测试返回前就已提交（4 × 5、无 MPS）；实测后在开跑前改为 2 × 10 + MPS（见 §1 的吞吐表），训练代码与配方不变；walltime 不够靠 `--resume` 续。
